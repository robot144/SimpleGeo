...
function test1()
   ...
   boundbox=[0.0,49.0,9.0,55.0] #avoid the use of the name bbox, since it may lead to side-effects
   img=get_map(emodnet_server,boundbox)
   @test size(img)==(800, 1200)
   @test Float64(img[1,1].r)==0.6941176470588235 #carefull, since this is sensitive to server changes
